...
describe 'DataServiceApi::QueryGenerator' do
  ...
  it 'should allow adding a relational operator clause' do
    ...
    query.ge('foo', 1).to_json.must_match_json_expression(conjunction(foo: { '@ge' => 1 }))
    query.gt('foo', 1).to_json.must_match_json_expression(conjunction(foo: { '@gt' => 1 }))
    query.le('foo', 1).to_json.must_match_json_expression(conjunction(foo: { '@le' => 1 }))
    query.lt('foo', 1).to_json.must_match_json_expression(conjunction(foo: { '@lt' => 1 }))
  end
